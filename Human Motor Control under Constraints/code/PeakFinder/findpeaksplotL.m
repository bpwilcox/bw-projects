function P=findpeaksplotL(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smoothtype)
% function P=findpeaks(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smoothtype)
% Function to locate and plot the positive peaks in a noisy x-y time series data
...
%   x=1:.2:100;
%   y=lorentzian(x,20,10)+lorentzian(x,50,10)+lorentzian(x,80,10)+.1.*randn(size(x));
%   findpeaksplotL(x,y,0.0004,0.3,17,19,3)
% Compare to findpeaksplot(x,y,0.0004,0.3,17,19,3) with the same signal.
% 
% Related functions:
