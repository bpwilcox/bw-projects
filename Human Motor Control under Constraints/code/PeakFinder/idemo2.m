...
% Peakfit functions applied to a test signal consisting of 16 peaks
% with random peak heights and gradually increasing peak widths.
%   T. C. O'Haver, July 2012
increment=1;
x=1:increment:4000;

% For each simulated peak, compute the amplitude, position, and width
pos=[400 900 1000 1100 1500 1700 2200 2400 2500 2700 2800 3100 3200 3300 3400 3800];   % Positions of the peaks (Change if desired)
amp=round(1+5.*rand(1,length(pos)));  % Amplitudes of the peaks  (Change if desired)
wid=60+60.*pos./10000;   % Widths of the peaks (Change if desired)
...
SmoothWidth=25;  % SmoothWidth should be roughly equal to 1/2 the peak width (in points)
...
tic;
iPeakResults=ipeak(demodata,0,AmpThreshold,SlopeThreshold,SmoothWidth,FitWidth,ActualPeaks(1,2),200,1);
TiPeak=toc;
disp('-----------------------------------------------------------------')
disp('Comparison of the iPeak and Peakfit functions for a test signal ')
disp('consisting of multiple peaks with random peak heights and gradually')
disp('increasing peak widths, superimposed on a curved sloping background. ')
disp(' ')
...
% Compare iPeakResults to ActualPeaks
disp('  ')
disp('Using iPeak:')
disp('>> iPeakResults=ipeak(demodata,0,AmpThreshold,SlopeThreshold,SmoothWidth,FitWidth,ActualPeaks(1,2),200,1);')
...
for n=1:SizeResults(1),
    indexn=val2ind(ActualPeaks(:,2),iPeakResults(n,2));
    error(n,:)=100.*abs(ActualPeaks(indexn,:)-iPeakResults(n,:))./ActualPeaks(indexn,:);
    error(n,1)=iPeakResults(n,1);
    % MeasuredPeaks(n,1)=indexn;
end
AverageiPeakError=mean(error)
disp(['Elapsed time is ' num2str(TiPeak) ' seconds'])
disp(' ')
disp('Comparing ActualPeaks to iPeakResults, you can see that iPeak')
disp('works well for isolated peaks (like peak 1), but does not')
disp('accurately correct for the background when the peaks are close ')
disp('together, like peak 2.  Also it sometimes misses the small peaks.')
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
pause
% -------------------------------------------------------------------
disp('  ')
disp('Using peakfit:')
disp('To use peakfit on a signal with lots of peaks, it is best to')
disp('fit each group of overlapping peaks separately. In this signal,')
disp('there are 8 groups ranging from 1 peak to 4 peaks per group.')
disp('  ')

% Group 1
disp('Group 1')
disp('>> FitResults=peakfit(demodata,400,265,1,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
figure(2)
PeakfitErrors=0;
tic;
FitResults=peakfit(demodata,400,265,1,1,0,10,0,1,0);
Tpeakfit1=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
    indexn=val2ind(ActualPeaks(:,2),FitResults(n,2));
    error(n,:)=100.*abs(ActualPeaks(indexn,:)-FitResults(n,:))./ActualPeaks(indexn,:);
    error(n,1)=indexn;
    FitResults(n,1)=indexn;
end
ActualPeaksInThisGroup=ActualPeaks(FitResults(:,1),:)
FitResults
if SizeResults(1)==1,
...
disp(['Elapsed time is ' num2str(Tpeakfit1) ' seconds'])
drawnow
PeakfitErrors=AveragePercentErrors;
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
pause
...
disp('  ')
disp('Group 2  ')
disp('>> FitResults=peakfit(demodata,1000,3,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic
FitResults=peakfit(demodata,1000,438,3,1,0,10,0,1,0);
Tpeakfit2=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit2) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
pause
...
disp('  ')
disp(' Group 3  ')
disp('>> FitResults=peakfit(demodata,1595,477,2,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,1595,477,2,1,0,10,0,1,0);
Tpeakfit3=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit3) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
...
disp('  ')
disp('Group 4  ')
disp('>> FitResults=peakfit(demodata,2187,246,1,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,2187,246,1,1,0,10,0,1,0);
Tpeakfit4=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit4) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
...
disp('  ')
disp('Group 5  ')
disp('>> FitResults=peakfit(demodata,2450,325,2,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,2450,325,2,1,0,10,0,1,0); 
Tpeakfit5=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit5) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
...
disp('  ')
disp('Group 6  ')
disp('>> FitResults=peakfit(demodata,2742,325,2,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,2742,325,2,1,0,10,0,1,0); 
Tpeakfit6=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit6) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
...
disp('  ')
disp('Group 7  ')
disp('>> FitResults=peakfit(demodata,3260,600,4,6,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,3260,600,4,6,0,10,0,1,0); 
Tpeakfit7=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit7) ' seconds'])
drawnow
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
disp(' ')
fprintf(2,'Press any key to continue.....or press Ctrl-C to abort.\n')
...
disp('  ')
disp('Group 8  ')
disp('>> FitResults=peakfit(demodata,3795,410,1,1,0,10,0,1,0);')
disp('         Peak #    Position      Height       Width        Area')
tic;
FitResults=peakfit(demodata,3795,410,1,1,0,10,0,1,0); 
Tpeakfit8=toc;
SizeResults=size(FitResults);
error=zeros(SizeResults(1),5);
for n=1:SizeResults(1),
...
AveragePercentErrors(1)=0;
AveragePercentErrors
disp(['Elapsed time is ' num2str(Tpeakfit8) ' seconds'])
PeakfitErrors=[PeakfitErrors;AveragePercentErrors];
TotalPeakfitComputationTime=Tpeakfit1+Tpeakfit2+Tpeakfit3+Tpeakfit4+Tpeakfit5+Tpeakfit6+Tpeakfit7+Tpeakfit8
disp('  ')
disp('In summary, we can compare the average percent errors and the')
disp('computation time of the iPeak and Peakfit methods:')
disp('  ')
AverageiPeakError=AverageiPeakError(2:5)
iPeakComputationTime=TiPeak
AveragePeakfitErrors=mean(PeakfitErrors);
AveragePeakfitErrors=AveragePeakfitErrors(2:5)
TotalPeakfitComputationTime
disp('  ')
disp('You can see that the errors of the peakfit method are quite a bit')
disp(['lower (by about a factor of ' num2str(round(mean(AverageiPeakError./AveragePeakfitErrors))) '), but that the execution time' ])
disp(['is longer (by about a factor of ' num2str(round(TotalPeakfitComputationTime/TiPeak)) ').' ])
disp('For both methods, the peak positions are measured most accurately and')
disp('the peak widths and areas least accurately.')
disp('  ')
disp('For a more challenging test, change line 13 to read Noise=.5 ')
disp('and re-run this script.')
