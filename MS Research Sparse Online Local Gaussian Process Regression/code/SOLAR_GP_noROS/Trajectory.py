...
class nPoly2D():
    ...
    def __init__(self,num_points, num_sides,  radius = 0.5, center_x = -0.25, center_y = 1, arclength = 2*np.pi, flip = 0,*args):
    ...
    def _createXtest(self):
        ...
        theta = np.linspace(0,self.arclength,self.num_sides+1)
        if self.flip:
            self.center_x += 2*self.radius 
            theta = theta+np.pi
        Xc = self.radius*np.cos(theta) + self.center_x
        Yc = self.radius*np.sin(theta) + self.center_y
