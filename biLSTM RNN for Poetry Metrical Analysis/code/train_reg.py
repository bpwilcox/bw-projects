...
train_english = list(english[:int(len(english)*0.8)])
train_meter = list(meter[:int(len(meter)*0.8)])

val_english = list(english[int(len(english)*0.8):])
val_meter = list(meter[int(len(meter)*0.8):])
...
y = np.array(train_meter[SEQ_LEN//2-1:-SEQ_LEN//2])
...
val_y = np.array(val_meter[SEQ_LEN//2-1:-SEQ_LEN//2])
...
left = Sequential()
left.add(LSTM(output_dim=layers[1], return_sequences=False, input_shape=(SEQ_LEN, len(english_sorted)), W_regularizer = l2(.01)))



right = Sequential()
right.add(LSTM(output_dim=layers[1], return_sequences=False,
               input_shape=(SEQ_LEN, len(english_sorted)), W_regularizer = l2(.01), go_backwards=True))
